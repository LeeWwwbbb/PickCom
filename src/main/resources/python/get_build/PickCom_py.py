...
def get_data(cursor, Choice_Where_to_Use):
    ...
    money_ranges = [400000, 600000, 700000, 800000, 1000000, 1300000, 2000000, 2400000, 3000000]
    # 50~60, 70~80, 90~110, 120~140, 180~200, 220~260, 270~330

    if Choice_Where_to_Use == 'S':
        # ssd 정보 가져오기
        ssd_query = "SELECT product_num, product_name, product_originalPrice FROM pc_ssd WHERE S_size IN ('250GB', '256GB', '500GB') ORDER BY product_num LIMIT 10"
        cursor.execute(ssd_query)
        S_ssd_data = cursor.fetchall()
        # power 정보 가져오기
        power_query = "SELECT product_num, product_name, product_originalPrice, W FROM pc_power ORDER BY product_num LIMIT 100"
    ...
    if Choice_Where_to_Use == 'G':
        ...
        # power 정보 가져오기
        G_power_query = "SELECT product_num, product_name, product_originalPrice, W FROM pc_power ORDER BY product_num LIMIT 100"
    ...
    for money_range in money_ranges:
                                ...
                                if Board_Size == mboard_MBoard_Size:
                                    for pc_ssd in S_ssd_data:
                                        ...
                                        if cpu_Stock_Cooler == "미포함":
                                            Cool_in_TotalPrice = int(cpu_product_originalPrice) + int(
                                                mboard_product_originalPrice) + int(ram_product_originalPrice) + int(
                                                cooler_product_originalPrice) + power_price + case_price + ssd_price
                                            ...
                                            Cool_out_TotalPrice = int(cpu_product_originalPrice) + int(
                                                mboard_product_originalPrice) + int(
                                                ram_product_originalPrice) + power_price + case_price + ssd_price
                                            if money_range * 0.9 <= Cool_out_TotalPrice <= money_range * 1.1:
                                                recommend_Val = int(
                                                    cpu_product_num + mboard_product_num + ram_product_num + ssd_rec + power_rec + case_rec) / 6
                                                if recommend_Val <= 30:
                                                    S_result_list.append({
                                                        "CPU": cpu_product_name,
                                                        "MBoard": mboard_product_name,
        ...
        if Choice_Where_to_Use == 'G':
                                ...
                                if case_vga_Size >= vga_Size and case_Mboard_Size == mboard_MBoard_Size:
                                    for pc_ssd in G_ssd_data:
                                        ...
                                        if cpu_Stock_Cooler == "미포함":
                                            Cool_in_TotalPrice = int(cpu_product_originalPrice) + int(
                                                mboard_product_originalPrice) + int(ram_product_originalPrice) + int(
                                                cooler_product_originalPrice) + int(
                                                pc_vga[2]) + power_price + case_price + ssd_price
                                            if money_range * 0.95 <= Cool_in_TotalPrice <= money_range * 1.05:
                                                recommend_Val = int(
                                                    cpu_product_num + mboard_product_num + ram_product_num
                                                    + cooler_product_num + ssd_rec + power_rec
                                                ...
                                                if recommend_Val <= 30:
                                                    ...
                                                    if len(added_vga_count) >= 100 or len(added_vga_count) >= len(
                                                            G_vga_data):
                                                        break
            ...
            G_result_list = [vga for vga in G_result_list if
